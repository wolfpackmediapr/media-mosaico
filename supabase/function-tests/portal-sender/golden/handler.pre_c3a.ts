/**
 * GOLDEN REFERENCE — DO NOT EDIT, NOT PART OF THE DEPLOYED RUNTIME.
 *
 * Byte-for-byte copy of `supabase/functions/portal-sender/handler.ts` at the
 * last pre-C3A revision (git 7324b65a, CP3 Phase F deployed source), with the
 * only modification being the relative import paths, which are rewritten to
 * point at the unchanged shared runtime modules (auth/clients/signing/finalize).
 *
 * The legacy clients response contract is derived from this file at test time
 * instead of from a hand-written key fixture.
 */
...
import { authorize, type AuthorizeResult } from "../../../functions/portal-sender/auth.ts";
import { chunk, fetchInternalClients, type ClientItemDTO } from "../../../functions/portal-sender/clients.ts";
import { SCHEMA_VERSION, SCHEMA_VERSION_HEADER, signRequest, verifyTestVector } from "../../../functions/portal-sender/signing.ts";
import {
  ...
  type FetchImpl,
  type FinalizeResult,
} from "../../../functions/portal-sender/finalize.ts";
