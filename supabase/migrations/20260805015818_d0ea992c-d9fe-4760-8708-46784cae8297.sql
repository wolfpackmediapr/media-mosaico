...
RETURNS TABLE(out_client_id uuid, out_client_category_id uuid, out_subcategory_ids uuid[])
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.client_categories cc WHERE cc.id = p_client_category_id) THEN
  ...
  INSERT INTO public.client_subcategory_assignments AS a (client_id, client_subcategory_id)
