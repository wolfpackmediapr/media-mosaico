...
export const importRatesFromCsv = async (
  ...
  try {
    ...
    // Required columns
    const requiredColumns = ['channel', 'program', 'days', 'start_time', 'end_time'];
    ...
    // Fetch all channels and programs to match by name
    const channels = await fetchChannels();
    const programs = await fetchPrograms();
    
    // Channel and program lookups by name
    const channelsByName = new Map(channels.map(s => [s.name.toLowerCase(), s]));
    const programsByName = new Map(programs.map(p => [p.name.toLowerCase(), p]));
    
    // Process each line (skip header)
    const ratesToImport: Omit<TvRateType, 'id' | 'created_at' | 'channel_name' | 'program_name'>[] = [];
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      try {
        ...
        const channelName = values[columnIndices.channel];
        ...
        if (!channelName || !programName || !daysString || !startTime || !endTime) {
          results.errors.push({
            line: i + 1,
        ...
        // Find or reject channel and program
